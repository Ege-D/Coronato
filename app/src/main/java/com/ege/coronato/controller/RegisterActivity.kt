...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerConfirmBtn.setOnClickListener {
            if (registerMailTxt.text.toString().isEmpty() || registerPwdTxt.text.toString()
                            .isEmpty() || registerNameTxt.text.toString().isEmpty() || registerPwdTxt2.text.toString().isEmpty())
                Toast.makeText(applicationContext,"Alanlar boş bırakılamaz.",Toast.LENGTH_SHORT).show()
            else if (registerPwdTxt.text.toString() != registerPwdTxt2.text.toString()) {
                Toast.makeText(applicationContext, "Şifreler aynı değil.", Toast.LENGTH_SHORT).show()
            }
            else {
